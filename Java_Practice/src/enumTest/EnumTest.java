...
/*
 	-enum(열거형)->서로 관련있는 상수들의 집합을 나타낸다.
 		->클래스처럼 보이게 하는 상수
 		->열거형은 클래스처럼 '독립된 java파일'에 만들 수 있고, 하나의 java클래스와 같이 만들 수 있고, 클래스 안에 내부 클래스처럼 만들 수 있다.
 	-열거형의 속성 및 메서드
 	1.name()->열거형 상수의 이름을 문자열로 반환한다.
 	2.ordinal()->열거형 상수가 정의된 순서값(index)을 반환한다.
 	...
 	방법2)각 상수에 임의의 값을 설정할 수 있다.
 		enum 열거형이름{
 			...
 			//'변수명'은 '값들'의 개수와 같고, 각각의 자료형이 맞아야한다.
...
public class EnumTest {
	...
	public static void main(String[] args) {
		...
		if(ConstTest.RED==ConstTest.THREE) System.out.println("zzzzz");
		else System.out.println("gggggg");
		...
		//열거형을 switch문으로 비교할 때는 case문에 지정하는 상수는 '열거형이름.상수명'에서 '열거형이름'을 생략한 '상수명'만 기술해야 한다.
		switch(mycol) {
		case RED:
